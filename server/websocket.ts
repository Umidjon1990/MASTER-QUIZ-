...
async function autoSendResultsToTelegram(
  ...
  leaderboard.forEach((r, i) => {
    ...
    const line = `${medal} ${i + 1}. ${r.name} — ${r.score} ball — ${r.correctAnswers}/${totalQuestions} to'g'ri (${pct}%)`;
    const isBold = i < 3;
    resultRows.push(new Paragraph({ children: [makeRun(line, isBold, 22)], spacing: { after: 80 }, bidirectional: hasRtl(r.name) }));
  });
