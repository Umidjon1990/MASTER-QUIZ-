...
import { registerAdminQuizAssignmentRoutes } from "./admin-quiz-assignment-routes";
...
export async function registerRoutes(
    ...
    try {
      ...
      res.json({
        ...
        questions: questionsList.map(q => {
          ...
          const section = sections.find((item: any) => questionNumber >= item.fromIndex && questionNumber <= item.toIndex);
          return {
          id: q.id,
          ...
          passage: section?.passageText ? { title: section.passageTitle || "", text: section.passageText } : null,
        };
        }),
      });
    } catch (error) {
  ...
  app.get("/api/quizzes/:quizId/questions", requireAuth, requireRole(["teacher", "admin"]), async (req: any, res) => {
    try {
      ...
      const sections: any[] = (quiz as any).questionSections || [];
      res.json(questionsList.map((question) => {
