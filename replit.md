...
- **Key tables**:
  ...
  - `ai_students` — AI sinf o'quvchilari (name, phone, telegramChatId). **Self-registration**: o'quvchilar bot orqali /start yuborib ism + telefon ni o'zlari kiritadilar va avtomatik ravishda ai_student record yaratiladi. Bulk import via textarea (har qatorda ism + telefon) qo'shimcha imkoniyat sifatida saqlangan. `paymentInfo` JSONB: {status: paid/unpaid/partial, amount, lessonsCount, untilDate, note}.
